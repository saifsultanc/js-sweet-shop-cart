  ...
  cartBtn.forEach(function(btn) {
    btn.addEventListener("click", function(event) {
      ...
      if (event.target.parentElement.classList.contains("store-item-icon")) {
        ...
        item.price = event.target.parentElement.parentElement.nextElementSibling.children[0].children[1].textContent
          .slice(2)
          .trim();
        //console.log(item);
...
function removeItem() {
  const itemRemoved = document.getElementById("cart-item-remove").parentElement
...
$("#checkout-cart").click(function() {
  ...
  swal({
    title: "Checkout Done!",
    text: "Please collect your order at checkout desk.",
    icon: "success",
    button: "Thanks!"
  });
});
